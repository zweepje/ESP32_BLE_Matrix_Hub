...
void IndexedBitmap::drawLine(int x0, int y0, int x1, int y1, uint8_t colorIndex) {
    int dx = abs(x1 - x0);
    int dy = -abs(y1 - y0);
    int sx = x0 < x1 ? 1 : -1;
    int sy = y0 < y1 ? 1 : -1;
    int err = dx + dy; // fout-accumulator

    while (true) {
        // Teken de huidige pixel
        setPixel(x0, y0, colorIndex);

        // Stop als we het eindpunt hebben bereikt
        if (x0 == x1 && y0 == y1) break;

        int e2 = 2 * err;
        if (e2 >= dy) { // stap in x-richting
            err += dy;
            x0 += sx;
        }
        if (e2 <= dx) { // stap in y-richting
            err += dx;
            y0 += sy;
        }
    }
